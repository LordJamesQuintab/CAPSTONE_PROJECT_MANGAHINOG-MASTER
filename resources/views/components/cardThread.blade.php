...
{{-- binago  --}}
@auth
<div class="block p-4 sm:p-6 border border-gray-200 rounded-lg shadow dark:border-gray-700 w-full mx-auto max-w-4xl">
    <a href="/threads/{{$thread['id']}}">
        ...
        <div class="ms-16">
            <h5 class="mb-2 text-xl md:text-2xl font-bold tracking-tight text-black">
                {{$thread['title']}}
            </h5>
            ...
            <div>
                @if($thread->image)
                    <img src="{{ asset('storage/' . $thread->image) }}" alt="Thread Image" class="w-full h-auto rounded-lg mt-4">
                @endif
            </div>
        </div>
    </a>
        ...
        <form class="flex gap-1" action="/threads/{{ $thread->id }}/like" method="POST">
            ...
            <button type="submit" class="flex items-center">
                ...
                @if($thread->likes->count() > 0)
                    <p class="ml-1">{{ $thread->likes->count() }}</p>
                @endif
            </button>
        </form>
